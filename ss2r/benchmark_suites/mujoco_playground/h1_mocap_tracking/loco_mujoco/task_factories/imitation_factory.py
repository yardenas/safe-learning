...
from ss2r.benchmark_suites.mujoco_playground.h1_mocap_tracking.loco_mujoco.datasets.humanoids.LAFAN1 import (
    LAFAN1_ALL_DATASETS,
    LAFAN1_DANCE_DATASETS,
    LAFAN1_LOCOMOTION_DATASETS,
    load_lafan1_trajectory,
)
from ss2r.benchmark_suites.mujoco_playground.h1_mocap_tracking.loco_mujoco.environments.base import (
    LocoEnv,
)
from ss2r.benchmark_suites.mujoco_playground.h1_mocap_tracking.loco_mujoco.smpl.const import (
    AMASS_LOCOMOTION_DATASETS,
)
from ss2r.benchmark_suites.mujoco_playground.h1_mocap_tracking.loco_mujoco.smpl.retargeting import (
    extend_motion,
    ...
    def make(
        ...
        **kwargs,
        ...
        if env_name not in LocoEnv.registered_envs:
            raise KeyError(
                f"Environment '{env_name}' is not a registered LocoMuJoCo environment."
            )
    ...
    def get_default_traj(env, default_dataset_conf) -> Trajectory:
        ...
        try:
            with open(path_to_conf, "r") as file:
                ...
                try:
                    path_to_convert_default_datasets = data[
        ...
        for task in default_dataset_conf.task:
            ...
            if path_to_convert_default_datasets:
                cached_file_path = os.path.join(
                    path_to_convert_default_datasets, file_path
                )
                if os.path.isfile(cached_file_path):
                    print(
                        f"[LocoMuJoCo's Default Dataset Pipeline] INFO: Found converted dataset at: {cached_file_path}."
                    )
            ...
            if traj is None:
                ...
                if not traj.data.is_complete:
                    traj = extend_motion(
                        env_name, load_robot_conf_file(env_name).env_params, traj
                    )
    ...
    def get_amass_traj(env, amass_dataset_conf: AMASSDatasetConf) -> Trajectory:
        ...
        if amass_dataset_conf.dataset_group is not None:
            if amass_dataset_conf.dataset_group == "AMASS_LOCOMOTION_DATASETS":
                ...
                raise ValueError(
                    f"Unknown dataset group: {amass_dataset_conf.dataset_group}"
                )
    ...
    def get_lafan1_traj(env, lafan1_dataset_conf: LAFAN1DatasetConf) -> Trajectory:
        ...
        if lafan1_dataset_conf.dataset_group:
            if lafan1_dataset_conf.dataset_group == "LAFAN1_LOCOMOTION_DATASETS":
                ...
                raise ValueError(
                    f"Unknown dataset group: {lafan1_dataset_conf.dataset_group}"
                )
        else:
            dataset_paths = (
