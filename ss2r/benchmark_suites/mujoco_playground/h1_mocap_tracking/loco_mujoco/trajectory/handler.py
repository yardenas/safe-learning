...
from ss2r.benchmark_suites.mujoco_playground.h1_mocap_tracking.loco_mujoco.core.stateful_object import (
    StatefulObject,
)
from ss2r.benchmark_suites.mujoco_playground.h1_mocap_tracking.loco_mujoco.trajectory.dataclasses import (
    Trajectory,
...
class TrajectoryHandler(StatefulObject):
    ...
    def __init__(
        ...
        assert (traj_path is not None) != (traj is not None), (
            "Please specify either traj_path or " "trajectory, but not both."
        )
        ...
        assert (
            fixed_start_conf is not None
        ) != random_start, "Please specify either fixed_start_conf or random_start."
        ...
        if self.traj_dt != self.control_dt:
            traj_data, traj_info = interpolate_trajectories(
                traj_data, traj_info, 1.0 / self.control_dt
            )
    ...
    def len_trajectory(self, traj_ind):
        return (
            self.traj.data.split_points[traj_ind + 1]
            - self.traj.data.split_points[traj_ind]
        )
    ...
    def filter_and_extend(traj_data, traj_info, model):
        ...
        if bodies_to_be_removed:
            traj_data = traj_data.remove_bodies(
                jnp.array(list(bodies_to_be_removed.values()))
            )
            traj_info = traj_info.remove_bodies(list(bodies_to_be_removed.keys()))
        if site_to_be_removed:
            traj_data = traj_data.remove_sites(
                jnp.array(list(site_to_be_removed.values()))
            )
        ...
        if traj_info.body_names is not None:
            for b_name in body_names:
                if b_name not in traj_info.body_names:
                    ...
                    traj_info = traj_info.add_body(
                        b_name,
                        model.body_rootid[b_id],
                        model.body_weldid[b_id],
                        model.body_mocapid[b_id],
                        model.body_pos[b_id],
                        model.body_quat[b_id],
                        model.body_ipos[b_id],
        ...
        if traj_info.site_names is not None:
            for s_name in site_names:
                if s_name not in traj_info.site_names:
                    ...
                    traj_info = traj_info.add_site(
                        s_name,
                        model.site_pos[s_id],
                        model.site_quat[s_id],
        ...
        traj_info = (
            traj_info.reorder_bodies(new_body_order)
            if traj_info.body_names is not None
            else traj_info
        )
        traj_info = (
            traj_info.reorder_sites(new_site_order)
            if traj_info.site_names is not None
            else traj_info
        )
        traj_data = traj_data.reorder_joints(
            jnp.concatenate(new_joint_order_ids_qpos),
            jnp.concatenate(new_joint_order_ids_qvel),
        )
        traj_data = (
            traj_data.reorder_bodies(jnp.array(new_body_order))
            if traj_info.body_names is not None
            else traj_data
        )
        traj_data = (
            traj_data.reorder_sites(jnp.array(new_site_order))
            if traj_info.site_names is not None
            else traj_data
        )
    ...
    def reset_state(self, env, model, data, carry, backend):
        ...
        return data, carry.replace(
            ...
            traj_state=TrajState(
                new_traj_no, new_subtraj_step_no, new_subtraj_step_no_init
            ),
        )
    ...
    def to_numpy(self):
        if not self._is_numpy:
            ...
            self.traj = replace(
                self.traj, data=self.traj.data.to_numpy(), info=traj_info
            )
            self._is_numpy = True
