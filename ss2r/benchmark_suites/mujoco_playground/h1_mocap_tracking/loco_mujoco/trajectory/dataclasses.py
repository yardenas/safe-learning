...
from ss2r.benchmark_suites.mujoco_playground.h1_mocap_tracking.loco_mujoco.core.observations.base import (
    ...
    def load(cls, path, backend: ModuleType = jnp):
        ...
        for key, value in data.items():
            if key in TrajectoryInfo.get_attribute_names():
                converted_info[key] = (
                    None if is_none_object_array(value) else value.tolist()
                )
            elif key in TrajectoryModel.get_attribute_names():
                converted_model[key] = (
                    None if is_none_object_array(value) else backend.array(value)
                )
        ...
        _all = {
            "data": TrajectoryData(**converted_data),
            "info": TrajectoryInfo(
                model=TrajectoryModel(**converted_model), **converted_info
            ),
        }
...
class TrajectoryInfo:
    ...
    def __post_init__(self):
        ...
        for i, item in enumerate(zip(self.joint_names, self.model.jnt_type)):
            ...
            if j_type == mujoco.mjtJoint.mjJNT_FREE:
                ...
                j_type == mujoco.mjtJoint.mjJNT_SLIDE
                or j_type == mujoco.mjtJoint.mjJNT_HINGE
            ):
    ...
    def __eq__(self, other, backend: ModuleType = jnp):
        ...
        for key in self.joint_name2ind_qpos:
            if not backend.array_equal(
                self.joint_name2ind_qpos[key], other.joint_name2ind_qpos[key]
            ):
                return False
        ...
        for key in self.joint_name2ind_qvel:
            if not backend.array_equal(
                self.joint_name2ind_qvel[key], other.joint_name2ind_qvel[key]
            ):
                return False
        ...
        for key in self.body_name2ind:
            if not backend.array_equal(
                self.body_name2ind[key], other.body_name2ind[key]
            ):
                return False
        ...
        for key in self.site_name2ind:
            if not backend.array_equal(
                self.site_name2ind[key], other.site_name2ind[key]
            ):
                return False
    ...
    def add_joint(self, joint_name, joint_type, backend: ModuleType = jnp):
        ...
        return replace(
            self, joint_names=self.joint_names + [joint_name], model=new_model
        )
    ...
    def add_body(
        ...
        return replace(self, body_names=self.body_names + [body_name], model=new_model)

    def add_site(
        self, site_name, site_pos, site_quat, site_bodyid, backend: ModuleType = jnp
    ):
        """
        ...
        return replace(self, site_names=self.site_names + [site_name], model=new_model)
    ...
    def remove_joints(self, joint_names, backend: ModuleType = jnp):
        ...
        """
        new_model = self.model.remove_joints(
            backend.array([self.joint_names.index(name) for name in joint_names]),
            backend,
        )
        return replace(
            self,
    ...
    def remove_bodies(self, body_names, backend: ModuleType = jnp):
        ...
        """
        new_model = self.model.remove_bodies(
            backend.array([self.body_name2ind[name] for name in body_names]), backend
        )
        return replace(
            self,
    ...
    def remove_sites(self, site_names, backend: ModuleType = jnp):
        ...
        """
        new_model = self.model.remove_sites(
            backend.array([self.site_name2ind[name] for name in site_names]), backend
        )
        return replace(
            self,
    ...
    def reorder_joints(self, new_order, backend: ModuleType = jnp):
        ...
        return replace(
            self, joint_names=[self.joint_names[i] for i in new_order], model=new_model
        )
    ...
    def reorder_bodies(self, new_order, backend: ModuleType = jnp):
        ...
        return replace(
            self, body_names=[self.body_names[i] for i in new_order], model=new_model
        )
    ...
    def reorder_sites(self, new_order, backend: ModuleType = jnp):
        ...
        return replace(
            self, site_names=[self.site_names[i] for i in new_order], model=new_model
        )
...
class TrajectoryModel:
    ...
    nbody: int = None
    body_rootid: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    body_weldid: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    body_mocapid: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    body_pos: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    body_quat: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    body_ipos: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    body_iquat: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )

    # site properties in Mujoco model
    nsite: int = None
    site_bodyid: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    site_pos: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    site_quat: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    ...
    def add_body(
        ...
        return self.replace(
            nbody=self.nbody + 1,
            body_rootid=backend.concatenate(
                [self.body_rootid, backend.array([body_rootid])]
            ),
            body_weldid=backend.concatenate(
                [self.body_weldid, backend.array([body_weldid])]
            ),
            body_mocapid=backend.concatenate(
                [self.body_mocapid, backend.array([body_mocapid])]
            ),
            ...
            body_iquat=backend.concatenate(
                [self.body_iquat, backend.array([body_iquat])]
            ),
        )
    ...
    def add_site(self, site_pos, site_quat, site_body_id, backend: ModuleType = jnp):
        ...
        return self.replace(
            nsite=self.nsite + 1,
            site_bodyid=backend.concatenate(
                [self.site_bodyid, backend.array([site_body_id])]
            ),
            site_pos=backend.concatenate([self.site_pos, backend.array([site_pos])]),
            site_quat=backend.concatenate(
                [self.site_quat, backend.array([site_quat])],
            ),
        )
    ...
    def reorder_joints(self, new_order, backend: ModuleType = jnp):
        ...
        return self.replace(jnt_type=self.jnt_type[new_order])
...
class SingleData:
    ...
    xpos: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    xquat: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    cvel: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    subtree_com: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )

    # global site properties
    site_xpos: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    site_xmat: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
...
class TrajectoryData(SingleData):
    ...
    split_points: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    ...
    def __eq__(self, other, backend: ModuleType = jnp):
        ...
        if not (
            backend.array_equal(self.qpos, other.qpos)
            and backend.array_equal(self.qvel, other.qvel)
            and backend.array_equal(self.xpos, other.xpos)
            and backend.array_equal(self.xquat, other.xquat)
            and backend.array_equal(self.cvel, other.cvel)
            and backend.array_equal(self.subtree_com, other.subtree_com)
            and backend.array_equal(self.site_xpos, other.site_xpos)
            and backend.array_equal(self.site_xmat, other.site_xmat)
            and backend.array_equal(self.split_points, other.split_points)
        ):
            return False
        return True
    ...
    def get(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        ...
        return SingleData(
            ...
            xpos=backend.squeeze(self.xpos[ind].copy())
            if self.xpos.size > 0
            else backend.empty((1, 0)),
            xquat=backend.squeeze(self.xquat[ind].copy())
            if self.xquat.size > 0
            else backend.empty((1, 0)),
            cvel=backend.squeeze(self.cvel[ind].copy())
            if self.cvel.size > 0
            else backend.empty((1, 0)),
            subtree_com=backend.squeeze(self.subtree_com[ind].copy())
            if self.subtree_com.size > 0
            else backend.empty((1, 0)),
            site_xpos=backend.squeeze(self.site_xpos[ind].copy())
            if self.site_xpos.size > 0
            else backend.empty((1, 0)),
            site_xmat=backend.squeeze(self.site_xmat[ind].copy())
            if self.site_xmat.size > 0
            else backend.empty((1, 0)),
        )
    ...
    def dynamic_slice_in_dim(
        ...
        return data.replace(
            qpos=backend.squeeze(
                cls._dynamic_slice_in_dim_compat(
                    data.qpos, slice_start, slice_length, backend
                )
            ),
            qvel=backend.squeeze(
                cls._dynamic_slice_in_dim_compat(
                    data.qvel, slice_start, slice_length, backend
                )
            ),
            xpos=backend.squeeze(
                cls._dynamic_slice_in_dim_compat(
                    data.xpos, slice_start, slice_length, backend
                )
            )
            if data.xpos.size > 0
            else backend.empty(0),
            xquat=backend.squeeze(
                cls._dynamic_slice_in_dim_compat(
                    data.xquat, slice_start, slice_length, backend
                )
            )
            if data.xquat.size > 0
            else backend.empty(0),
            cvel=backend.squeeze(
                cls._dynamic_slice_in_dim_compat(
                    data.cvel, slice_start, slice_length, backend
                )
            )
            if data.cvel.size > 0
            else backend.empty(0),
            subtree_com=backend.squeeze(
                cls._dynamic_slice_in_dim_compat(
                    data.subtree_com, slice_start, slice_length, backend
                )
            )
            if data.subtree_com.size > 0
            else backend.empty(0),
            site_xpos=backend.squeeze(
                cls._dynamic_slice_in_dim_compat(
                    data.site_xpos, slice_start, slice_length, backend
                )
            )
            if data.site_xpos.size > 0
            else backend.empty(0),
            site_xmat=backend.squeeze(
                cls._dynamic_slice_in_dim_compat(
                    data.site_xmat, slice_start, slice_length, backend
                )
            )
            if data.site_xmat.size > 0
            else backend.empty(0),
    ...
    @staticmethod
    def _get_single_attribute(
        attribute, split_points, traj_index, sub_traj_index, backend
    ):
        """
    ...
    def _dynamic_slice_in_dim_single(
        self, attribute, split_points, traj_index, sub_traj_index, slice_length, backend
    ):
        """
        ...
        return backend.squeeze(
            self._dynamic_slice_in_dim_compat(
                attribute, slice_start, slice_length, backend
            )
        )

    def get_qpos(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        return self._get_single_attribute(
            self.qpos, self.split_points, traj_index, sub_traj_index, backend
        )

    def get_qvel(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        return self._get_single_attribute(
            self.qvel, self.split_points, traj_index, sub_traj_index, backend
        )

    def get_xpos(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        return self._get_single_attribute(
            self.xpos, self.split_points, traj_index, sub_traj_index, backend
        )

    def get_xquat(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        return self._get_single_attribute(
            self.xquat, self.split_points, traj_index, sub_traj_index, backend
        )

    def get_cvel(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        return self._get_single_attribute(
            self.cvel, self.split_points, traj_index, sub_traj_index, backend
        )

    def get_subtree_com(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        return self._get_single_attribute(
            self.subtree_com, self.split_points, traj_index, sub_traj_index, backend
        )

    def get_site_xpos(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        return self._get_single_attribute(
            self.site_xpos, self.split_points, traj_index, sub_traj_index, backend
        )

    def get_site_xmat(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        return self._get_single_attribute(
            self.site_xmat, self.split_points, traj_index, sub_traj_index, backend
        )

    def get_qpos_slice(
        self, traj_index, sub_traj_index, slice_length, backend: ModuleType = jnp
    ):
        return self._dynamic_slice_in_dim_single(
            self.qpos,
            self.split_points,
            traj_index,
            sub_traj_index,
    ...
    def add_joint(self, qpos_value=0.0, qvel_value=0.0, backend: ModuleType = jnp):
        ...
        """
        return self.replace(
            qpos=backend.concatenate(
                [self.qpos, backend.full((self.qpos.shape[0], 1), qpos_value)], axis=1
            ),
            qvel=backend.concatenate(
                [self.qvel, backend.full((self.qvel.shape[0], 1), qvel_value)], axis=1
            ),
        )
    ...
    def add_body(
        ...
        """
        quats = backend.broadcast_to(
            backend.array([1.0, 0.0, 0.0, 0.0]), (self.xquat.shape[0], 1, 4)
        )
        return self.replace(
            xpos=backend.concatenate(
                [self.xpos, backend.full((self.xpos.shape[0], 1, 3), xpos_value)],
                axis=1,
            ),
            xquat=backend.concatenate([self.xquat, quats], axis=1),
            cvel=backend.concatenate(
                [self.cvel, backend.full((self.cvel.shape[0], 1, 6), cvel_value)],
                axis=1,
            ),
            subtree_com=backend.concatenate(
                [
                    self.subtree_com,
                    backend.full((self.subtree_com.shape[0], 1, 3), subtree_com_value),
                ],
                axis=1,
            ),
        )
    ...
    def add_site(self, site_xpos_value=0.0, backend: ModuleType = jnp):
        ...
        """
        return self.replace(
            site_xpos=backend.concatenate(
                [
                    self.site_xpos,
                    backend.full((self.site_xpos.shape[0], 1, 3), site_xpos_value),
                ],
                axis=1,
            ),
            site_xmat=backend.concatenate(
                [
                    self.site_xmat,
                    backend.broadcast_to(
                        backend.eye(3).flatten(), (self.site_xmat.shape[0], 1, 9)
                    ),
                ],
                axis=1,
            ),
    ...
    def reorder_joints(self, new_order_qpos, new_order_qvel):
        ...
        """
        return self.replace(
            qpos=self.qpos[:, new_order_qpos], qvel=self.qvel[:, new_order_qvel]
        )
    ...
    def concatenate(traj_datas: list, traj_infos: list, backend: ModuleType = jnp):
        ...
        """
        assert len(traj_datas) == len(
            traj_infos
        ), "TrajectoryData and TrajectoryInfo must have the same length!"

        # assert that all traj_infos are equivalent
        assert all([info == traj_infos[0] for info in traj_infos]), (
            "TrajectoryInfos must be equivalent" " for concatenation!"
        )
        ...
        new_split_points = backend.concatenate(
            new_split_points + [backend.array([curr_n_samples])], axis=0
        )
        ...
        new_traj_data = TrajectoryData(
            ...
            subtree_com=backend.concatenate(
                [data.subtree_com for data in traj_datas], axis=0
            ),
            site_xpos=backend.concatenate(
                [data.site_xpos for data in traj_datas], axis=0
            ),
            site_xmat=backend.concatenate(
                [data.site_xmat for data in traj_datas], axis=0
            ),
...
def interpolate_trajectories(
    ...
    def interpolate_xmat(xmats, times, new_times):
        ...
        for i in range(traj_data_slice.site_xmat.shape[1]):
            ...
            xmat_interpolated = (
                Rotation.from_quat(xquat_interpolated).as_matrix().reshape(-1, 9)
            )
    ...
    for i in range(traj_data.n_trajectories):
        ...
        x_new = backend.linspace(
            0, traj_len - 1, round(traj_len * new_traj_sampling_factor), endpoint=True
        )

        # quaternions need SLERP interpolation
        if traj_data_slice.xquat.size > 0:
            xquat_interpolated = backend.stack(
                [
                    slerp_batch(traj_data_slice.xquat[:, i, :], x, x_new)
                    for i in range(traj_data_slice.xquat.shape[1])
                ],
                axis=1,
            )
        else:
        ...
        qpos_free_joint_quat_ids = [
            i[3:] for i in traj_info.joint_name2ind_qpos.values() if len(i) > 1
        ]
        qpos_free_joint_quat_ids_flat = [
            item for sublist in qpos_free_joint_quat_ids for item in sublist
        ]
        qpos_other_ids = backend.array(
            [
                i
                for i in range(traj_data.qpos.shape[-1])
                if i not in qpos_free_joint_quat_ids_flat
            ]
        )
        qpos = jnp.zeros((x_new.shape[0], traj_data_slice.qpos.shape[-1]))
        qpos = qpos.at[:, qpos_other_ids].set(
            interp1d(x, traj_data_slice.qpos[:, qpos_other_ids], kind="cubic", axis=0)(
                x_new
            )
        )
        for quat_ids in qpos_free_joint_quat_ids:
            quat_ids = backend.array(quat_ids)
            qpos = qpos.at[:, quat_ids].set(
                slerp_batch(traj_data_slice.qpos[:, quat_ids], x, x_new)
            )

        # interpolate the rest of the data
        qvel_interpolated = interp1d(x, traj_data_slice.qvel, kind="cubic", axis=0)(
            x_new
        )
        xpos_interpolated = (
            interp1d(x, traj_data_slice.xpos, kind="cubic", axis=0)(x_new)
            if traj_data_slice.xpos.size > 0
            else jnp.empty(0)
        )
        cvel_interpolated = (
            interp1d(x, traj_data_slice.cvel, kind="cubic", axis=0)(x_new)
            if traj_data_slice.cvel.size > 0
            else jnp.empty(0)
        )
        site_xpos_interpolated = (
            interp1d(x, traj_data_slice.site_xpos, kind="cubic", axis=0)(x_new)
            if traj_data_slice.site_xpos.size > 0
            else jnp.empty(0)
        )
        subtree_com_interpolated = (
            interp1d(x, traj_data_slice.subtree_com, kind="cubic", axis=0)(x_new)
            if traj_data_slice.subtree_com.size > 0
            else jnp.empty(0)
        )
    ...
    new_traj_data, traj_info = TrajectoryData.concatenate(
        new_traj_datas, [traj_info] * len(new_traj_datas), backend
    )
...
class TrajectoryTransitions:
    ...
    actions: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
    rewards: Union[jax.Array, np.ndarray] = struct.field(
        default_factory=lambda: jnp.empty(0)
    )
